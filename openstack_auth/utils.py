...
import swiftclient
from swiftclient.contrib.federated.federated import getRealmList, getIdPRequest, load_protocol_module, getUnscopedToken
...
def get_federated_keystone_url():
    federated_keystone_url = getattr(settings, 'OPENSTACK_FEDERATED_KEYSTONE_URL', 
                    "http://fedkeystone.sec.cs.kent.ac.uk:5000/v2.0/")
    return federated_keystone_url
...
def get_tenant_name(realm):
    federated_keystone_url = get_federated_keystone_url()
    request = getIdPRequest(federated_keystone_url, realm)
    ...
    tenantData = getUnscopedToken(federated_keystone_url, response, realm)
    for tenant in tenantData['tenants']:
        if tenant.get("project"):
